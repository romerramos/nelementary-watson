-- Nelson Elementary Watson - Main module
local M = {}
...
function M.setup(opts)
  ...
  if config.options.debug then
    print("Nelson Elementary Watson: Plugin initialized")
  end
end
...
function M.attach_to_buffer()
  ...
  if config.options.debug then
    print("Nelson Elementary Watson: Attached to buffer " .. buf)
  end
end
...
function M.schedule_update(buf)
  ...
  timer:start(config.options.update_delay, 0, vim.schedule_wrap(function()
    M.process_buffer(buf)
    timer:close()
    timer = nil
  end))
...
function M.process_buffer(buf)
  ...
  if not workspace_root then
    if config.options.debug then
      print("Nelson Elementary Watson: No workspace root found")
    end
    return
  end
  ...
  if not translations then
    if config.options.debug then
      print("Nelson Elementary Watson: No translations found for locale " .. current_locale)
    end
    decorator.clear_buffer(buf)
    return
  end
  ...
  if config.options.debug then
    print(string.format("Nelson Elementary Watson: Processed %d translation calls, displayed %d", #calls, #results))
  end
end
...
function M.change_locale()
  ...
  if not workspace_root then
    print("Nelson Elementary Watson: No workspace found")
    return
  end
  ...
  if #available_locales == 0 then
    print("Nelson Elementary Watson: No locales found")
    return
  end
  ...
  vim.ui.select(available_locales, {
    ...
    if choice and choice ~= current_locale then
      ...
      print("Nelson Elementary Watson: Locale changed to " .. choice)
    end
  end)
end

-- Create user commands
vim.api.nvim_create_user_command("NelsonChangeLocale", M.change_locale, {
